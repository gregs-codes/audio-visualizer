...
// Shader code from the original project
const vertexShader = `
  uniform float u_time;
  uniform float u_frequency;
  varying vec3 vNormal;

  // Perlin noise helpers (from the original)
...
interface ThreeShaderVisualizerProps {
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      if (analyser && dataArray) {
        ...
        // Use average frequency for deformation
        const avg = dataArray.reduce((a, b) => a + b, 0) / dataArray.length;
        uniforms.u_frequency.value = avg;
      }
      // Update color uniforms if changed
