...
/*
  Hexagon HD (original):
  - Procedural hex tiling with neon outlines and cell fills.
  - Motion is slow and gated by beat; bass/energy modulate glow and hue.
  - Supports top/side tilt for parallax.
*/
...
const fragment = /* glsl */ `
  ...
  // Signed distance to a centered regular hex of radius r
  ...
  // Axial <-> pixel conversions for hex grids
  vec2 axialToPixel(vec2 qr, float s){
    float x = s * (sqrt(3.0) * qr.x + sqrt(3.0)/2.0 * qr.y);
    float y = s * (3.0/2.0 * qr.y);
    return vec2(x, y);
  }
  vec2 pixelToAxial(vec2 p, float s){
    float q = (sqrt(3.0)/3.0 * p.x - 1.0/3.0 * p.y) / s;
    float r = (2.0/3.0 * p.y) / s;
    return vec2(q, r);
  }
  // Round axial coords to nearest hex center via cube rounding
  vec2 axialRound(vec2 qr){
    float x = qr.x; float z = qr.y; float y = -x - z;
    float rx = round(x); float ry = round(y); float rz = round(z);
  ...
  void main(){
    // Coordinates and subtle view tilt
    vec2 uv = vUv; float aspect = u_res.x / max(1.0, u_res.y); uv.x *= aspect;
    uv.y = mix(uv.y, uv.y * 0.90 + (uv.x - 0.5*aspect) * 0.06, u_viewTilt);

    // Centered coordinates in world space
    vec2 p = (uv - vec2(0.5*aspect, 0.5)) * 2.4;

    float beat = smoothstep(0.0, 1.0, u_pulse);
    float s = 0.22; // hex size (radius)
    // Slow drift driven by beat
    p += vec2(sin(u_time * 0.15), cos(u_time * 0.12)) * (0.15 + 0.35 * beat);

    // Find nearest hex center in axial space
    vec2 qr  = pixelToAxial(p, s);
    vec2 hqr = axialRound(qr);
    vec2 hc  = axialToPixel(hqr, s);
    vec2 lp  = p - hc; // local position in hex cell

    // Distance to hex edge
    float d = sdHex(lp, s * 0.98);

    // Edge thickness and inner fill driven by audio
    float thick = mix(0.008, 0.003, clamp(u_bass * 0.8 + beat * 0.6, 0.0, 1.0));
    float edge  = 1.0 - smoothstep(thick, thick + 0.0025, abs(d));
    float fill  = smoothstep(0.15, -0.01, d);

    // Cell-wise flicker and flow along edges
    float cellNoise = hash12(hqr + floor(u_time));
    float flow = 0.5 + 0.5 * sin(u_time * (0.6 + 0.8 * u_energy) + cellNoise * 6.2831);
    float glow = (0.35 + 0.65 * beat) * (0.35 + 0.35 * u_bass + 0.25 * u_energy);

    // Warm honeycomb palette
    float hue = mix(0.06, 0.12, flow); // amber to yellow
    float sat = 0.85;
    float lum = 0.12 + 0.55 * fill;
    vec3 base = hsl2rgb(vec3(hue, sat, lum));

    // Combine fill + edge neon
    vec3 color = base + edge * glow * vec3(1.0, 0.75, 0.35);

    gl_FragColor = vec4(color, 1.0);
  }
`;

export async function renderHighGfxHexagonWithFeatures(
  key: string,
  width: number,
  height: number,
